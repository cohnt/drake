...
void Gemm<transpose>::CalcDV(const MatrixRef<double>& D,
  ...
  // (or row, when in transpose mode) and index `j` to refer to a row of the
  // D matrix (or column, when in transpose mode).
  const int max_i = !transpose ? V.cols() : V.rows();
  const int max_j = !transpose ? D.rows() : D.cols();

  // We'll use index `k` to index over the inner dot product.
  const int max_k = !transpose ? D.cols() : V.cols();
  ...
  // or V(j, k) when in transpose mode.
  ...
  for (int i = 0; i < max_i; ++i) {
    ...
    for (int k = 0; k < max_k; ++k) {
      const Variable& var = !transpose ? V(k, i) : V(i, k);
    ...
    for (int j = 0; j < max_j; ++j) {
      ...
      // Sum up D(j, k) * V(k, i) or in transpose mode V(j, i) * D(k, j).
      for (int k = 0; k < max_k; ++k) {
        inner_coeffs[var_index[k]] += !transpose ? D(j, k) : D(k, j);
      }
