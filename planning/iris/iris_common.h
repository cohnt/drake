...
T ScalarClip(const T& val, double a, double b) {
  if constexpr (std::is_same_v<T, drake::AutoDiffXd>) {
    Eigen::VectorXd zero_deriv = Eigen::VectorXd::Zero(val.derivatives().size());
    T a_ad(a, zero_deriv);
    T b_ad(b, zero_deriv);
...
template <typename T>
Eigen::VectorX<T> IiwaBimanualParameterization(const Eigen::VectorX<T>& q_and_psi, const bool shoulder_up, const bool elbow_up, const bool wrist_up, Eigen::VectorX<T>* unclipped_vals) {
  ...
  // Build list of transforms T_01, T_12, T_23 using Ts[0], Ts[1], Ts[2] and theta_[0:3]
