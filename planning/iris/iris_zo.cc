...
std::vector<CouplerConstraintSpec> TopologicalSortCouplerConstraints(const std::map<multibody::MultibodyConstraintId, CouplerConstraintSpec>& coupler_constraints) {
  std::map<JointIndex, JointIndex> next_map;   // joint0 -> joint1
  std::map<JointIndex, CouplerConstraintSpec> node_map; // joint1 -> constraint
  std::map<JointIndex, bool> has_predecessor;  // Tracks if a joint has an incoming connection

  // Step 1: Populate maps.
  for (const auto& [_, constraint] : coupler_constraints) {
      next_map[constraint.joint0_index] = constraint.joint1_index;
      node_map[constraint.joint1_index] = constraint;
      has_predecessor[constraint.joint1_index] = true;
      has_predecessor[constraint.joint0_index] = has_predecessor[constraint.joint0_index]; // Ensure it exists
  }

  // Step 2: Find the start node (a joint that is never a joint1_index).
  JointIndex start_joint{-1};
  for (const auto& [_, constraint] : coupler_constraints) {
      if (!has_predecessor[constraint.joint0_index]) {
          start_joint = constraint.joint0_index;
          break;
      }
  }
...
IrisZoOptions IrisZoOptions::CreateWithMimicJointsParameterization(
  ...
  const std::vector<multibody::JointIndex>& joint_indices = plant.GetJointIndices();
  ...
  for (const auto& [_, coupler_constraint] : coupler_constraints) {
    dependent_joint_indices.insert(joint_index_to_position_start.at(coupler_constraint.joint1_index));
  }
  ...
  std::set_difference(all_joint_indices.begin(), all_joint_indices.end(), dependent_joint_indices.begin(), dependent_joint_indices.end(), std::inserter(independent_joint_indices, independent_joint_indices.end()));

  std::vector<multibody::internal::CouplerConstraintSpec> sorted_coupler_constraints = TopologicalSortCouplerConstraints(coupler_constraints);

  // Construct the lambda.
  auto evaluate_parameterization = [independent_joint_indices, sorted_coupler_constraints, joint_index_to_position_start](const Eigen::VectorXd& q_in) {
    DRAKE_ASSERT(ssize(independent_joint_indices) == q_in.size());
    const int dimension_out = ssize(independent_joint_indices) + ssize(sorted_coupler_constraints);
    ...
    for (int j = 0; j < ssize(sorted_coupler_constraints); ++j) {
      ...
      int in_index = joint_index_to_position_start.at(coupler_constraint.joint0_index);
      int out_index = joint_index_to_position_start.at(coupler_constraint.joint1_index);
      q_out[out_index] = q_out[in_index] * coupler_constraint.gear_ratio + coupler_constraint.offset;
    }
    return q_out;
  };
